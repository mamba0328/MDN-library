...
exports.bookinstance_update_get = asyncHandler(async (req, res, next) => {
    const [bookInstance, allBooks] = await Promise.all([BookInstance.findById(req.params.id), Book.find({}, "title").exec()]);
    if(bookInstance === null){
        const error = new Error('No Bookinstance with such id');
        error.code = 404
        next(error);
    }
